...
void main()
{
    ...
    vec4 reflectionColor = texture(environmentMap, reflectDir);

    // Blend reflection with lighting result
    vec3 resultWithReflection = mix(result, reflectionColor.rgb, refVal);

    // Output the final color
    FragColor = vec4(resultWithReflection, alphaValue);
}
